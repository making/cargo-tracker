...
import lol.maki.cargotracker.domain.model.cargo.Delivery;
...
public class CargoTrackingDTOConverter {
	...
	public static CargoTrackingDTO convert(Cargo cargo, List<HandlingEvent> handlingEvents, MessageSource messageSource,
		...
		return new CargoTrackingDTO(convertTrackingId(cargo), convertStatusText(cargo, messageSource, locale),
				convertDestination(cargo), convertEta(cargo), convertNextExpectedActivity(cargo),
				convertIsMisdirected(cargo), handlingEventDTOs);
	}

	private static List<HandlingEventDTO> convertHandlingEvents(List<HandlingEvent> handlingEvents, Cargo cargo,
			MessageSource messageSource, Locale locale) {
		return handlingEvents.stream()
			.map(he -> new HandlingEventDTO(convertLocation(he), convertTime(he), convertType(he),
					convertVoyageNumber(he), convertIsExpected(he, cargo),
					convertDescription(he, messageSource, locale)))
			.collect(Collectors.toList());
	}

	protected static String convertDescription(HandlingEvent handlingEvent, MessageSource messageSource,
			Locale locale) {
		Object[] args;
		...
		switch (handlingEvent.type()) {
			case LOAD:
			case UNLOAD:
				args = new Object[] { handlingEvent.voyage().voyageNumber().idString(), handlingEvent.location().name(),
						formatter.format(handlingEvent.completionTime()) };
				break;
			case RECEIVE:
			case CUSTOMS:
			case CLAIM:
				args = new Object[] { handlingEvent.location().name(),
						formatter.format(handlingEvent.completionTime()) };
				break;
	...
	protected static String convertNextExpectedActivity(Cargo cargo) {
		...
		if (type.sameValueAs(HandlingEvent.Type.LOAD)) {
			return text + type.name().toLowerCase() + " cargo onto voyage " + activity.voyage().voyageNumber() + " in "
					+ activity.location().name();
		}
		else if (type.sameValueAs(HandlingEvent.Type.UNLOAD)) {
			return text + type.name().toLowerCase() + " cargo off of " + activity.voyage().voyageNumber() + " in "
					+ activity.location().name();
		}
		else {
