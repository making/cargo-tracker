...
import lol.maki.cargotracker.domain.model.cargo.HandlingActivity;
...
import lol.maki.cargotracker.domain.model.voyage.Schedule;
...
class CargoTrackingDTOConverterTest {
	...
	void shouldConvertValidCargoToDTO() {
		...
		assertThat(result)
			.extracting("trackingId", "statusText", "destination", "nextExpectedActivity", "isMisdirected")
			.contains("TEST123", "TEST-STATUS", "Helsinki", "Next expected activity is to receive cargo in Stockholm",
					false);
		// assertThat(result.getEta()).isEqualTo("1970-01-01 00:00"); // TODO test this
		// once we have added handling of timezones
	}
	...
	void shouldConvertValidCargoWithHandlingEventsToDTO() {
		...
		Voyage voyage = new Voyage(new VoyageNumber("0101"),
				new Schedule(List.of(new CarrierMovement(origin, dest, Instant.now(), Instant.now()))));
	...
	@ParameterizedTest
	void shouldConvertDescriptionCorrectlyForGivenParamsWithVoyage(String eventType, String expectedOutput)
			throws IOException {
		Voyage voyage = exampleVoyage();
		Instant date = Instant.parse("2022-11-01T13:37").atZone(ZoneOffset.UTC).toInstant();
		HandlingEvent event = new HandlingEvent(exampleCargo(), date, date, HandlingEvent.Type.valueOf(eventType),
				exampleLocation, voyage);
	...
	@ParameterizedTest
	void shouldConvertDescriptionCorrectlyForGivenParamsWithoutVoyage(String eventType, String expectedOutput)
			throws IOException {
		Instant date = Instant.parse("2022-11-01T13:37").atZone(ZoneOffset.UTC).toInstant();
		HandlingEvent event = new HandlingEvent(exampleCargo(), date, date, HandlingEvent.Type.valueOf(eventType),
				exampleLocation);
	...
	private static class NextExpectedActivityArgsProvider implements ArgumentsProvider {
		...
		public Stream<? extends Arguments> provideArguments(ExtensionContext context) throws Exception {
			Location origin = new Location(new UnLocode("SESTO"), "Stockholm");
			return Stream.of(
					Arguments.of(new HandlingActivity(HandlingEvent.Type.LOAD, origin, exampleVoyage()),
							"Next expected activity is to load cargo onto voyage 0101 in Stockholm"),
					Arguments.of(new HandlingActivity(HandlingEvent.Type.UNLOAD, origin, exampleVoyage()),
							"Next expected activity is to unload cargo off of 0101 in Stockholm"),
					Arguments.of(new HandlingActivity(HandlingEvent.Type.RECEIVE, origin),
							"Next expected activity is to receive cargo in Stockholm"),
					Arguments.of(new HandlingActivity(HandlingEvent.Type.CLAIM, origin),
							"Next expected activity is to claim cargo in Stockholm"),
					Arguments.of(new HandlingActivity(HandlingEvent.Type.CUSTOMS, origin),
	...
	private static Voyage exampleVoyage() {
		Location origin = new Location(new UnLocode("SESTO"), "Stockholm");
		Location dest = new Location(new UnLocode("FIHEL"), "Helsinki");
		return new Voyage(new VoyageNumber("0101"),
				new Schedule(List.of(new CarrierMovement(origin, dest, Instant.now(), Instant.now()))));
	}
