package lol.maki.cargotracker.acceptance.pages;
...
public class AdminPage {
	...
	public AdminPage(WebDriver driver, int port) {
		this.driver = driver;
		this.port = port;
		driver.get("http://localhost:%d/admin/list".formatted(port));
	...
	public boolean listedCargoContains(String expectedTrackingId) {
		...
		Optional<WebElement> matchingCargo = cargoList.stream()
			.filter(cargo -> cargo.getText().equals(expectedTrackingId))
			.findFirst();
